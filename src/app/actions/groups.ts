...
export async function getGroupPosts(groupId: string, cursor?: string): Promise<Post[]> {
  ...
  const [{ data: likeCounts }, { data: commentCounts }, { data: myLikes }, { data: sharedPostsData }] = await Promise.all([
    ...
    sharedPostIds.length > 0
      ? admin.from('posts').select('*, author:profiles!author_id(*), images:post_images(*), event:events!event_id(id, type, title, slug, starts_at, city, state, going_count, cover_photo_url, flyer_url, status), place:places!place_id(*)').in('id', sharedPostIds)
