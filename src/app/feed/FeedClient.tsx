...
export default function FeedClient({ currentUserId, currentUserProfile, userGroupIds = [], blockedUserIds = [], initialRiders = [], friendCount = 0, userBikes = [] }: Props) {
  ...
  const fetchPosts = useCallback(
    async (cursor?: string): Promise<{ posts: Post[]; rawCursor: string | null; rawFull: boolean }> => {
      ...
      let base = supabase
        .from('posts')
        .select('*, author:profiles!author_id(*), images:post_images(*), group:groups!group_id(name, slug, cover_photo_url), event:events!event_id(id, type, title, slug, starts_at, city, state, going_count, cover_photo_url, flyer_url, status), bike:user_bikes!bike_id(id, year, make, model, photo_url)')
        .is('deleted_at', null)
