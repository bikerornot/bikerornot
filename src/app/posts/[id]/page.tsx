...
export default async function PostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: post } = await supabase
    .from('posts')
    .select('*, author:profiles!author_id(*), images:post_images(*), group:groups!group_id(name, slug), place:places!place_id(*)')
    .eq('id', id)
    .is('deleted_at', null)
    .single()
  ...
  const [
    ...
    supabase.from('post_likes').select('post_id, user:profiles!user_id(status)').eq('post_id', id),
    ...
    post.shared_post_id
      ? supabase
          .from('posts')
          .select('*, author:profiles!author_id(*), images:post_images(*), event:events!event_id(id, type, title, slug, starts_at, city, state, going_count, cover_photo_url, flyer_url, status), place:places!place_id(*)')
  ...
  const enrichedPost: Post = {
    ...post,
    like_count: (likeCounts ?? []).filter((l: any) => !['banned', 'suspended'].includes(l.user?.status)).length,
